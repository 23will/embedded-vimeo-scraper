...
def run(url, skip_write_csv, skip_download_videos):
    ...
    if skip_write_csv is None or not skip_write_csv:
        ...
        video_csv_writer = VideoCsvWriter(settings.CSV_FILEPATH, html_cache, settings.URL_EXCLUDE_LIST)
        video_csv_writer.write_videos_to_csv(url)
...
run(args.url, args.skip_csv_writing, args.skip_video_download)
