...
class Videofinder:
    ...
    async def __find_videos_and_links(self, url, domain, parent_url, visited_urls):
        ...
        if html is not None:
            ...
            for video_url, title in video_url_and_titles.items():
                if video_url not in visited_urls or any(s in video_url for s in exception_url_parts):
    ...
    async def __find_videos(self, starting_url):
        ...
        while queue:
            ...
            videos, links = await self.__find_videos_and_links(url, domain, parent_url, visited_urls)
