...
class HtmlParser:
    ...
    def find_urls(self):
        ...
        for a_tag in a_tags:
            href = a_tag.get('href')
            if href is not None and not list(filter(lambda x: x in href, self.__excluded_urls)) and href.startswith(self.__url):
                hrefs.append(href)
        return hrefs


    def find_video_urls_and_titles(self, visited_video_urls):
        link_and_titles = dict()
        iframes = self.__soup.find_all('iframe')
        for a_tag in iframes:
            h1_tag = a_tag.find_previous('h1')
            src = a_tag.get('src')
            if src is not None and src not in visited_video_urls and 'vimeo' in src:
                tag_string = None if h1_tag is None else h1_tag.string
                link_and_titles[src] = None if tag_string is None else tag_string.strip()
