...
class EmbeddedVimeo:
    ...
    def _find_player_config_json(self, html):
        ...
        if title and title[0].string == 'Sorry':
            ...
            player_config_pattern = re.compile(r'\s*window\.playerConfig\s*=\s*(.*)', re.DOTALL)
    ...
    def _get_vimeo_download_info(self, master_json_url):
        ...
        download_info = VimeoDownloadInfo.from_master_json(response.json(), master_json_url)
    ...
    def _download(self, output_file, download_info):
        ...
        subprocess.call(f"ffmpeg -i {filename_video} -i {filename_audio} -c:v copy -c:a aac -b:a {download_info.audio_bitrate} '{output_file}'", shell=True)
